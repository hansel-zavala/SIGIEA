...
export const exportLecciones = async (status: string, format: string, res: Response) => {
  const where = buildWhereClause(status);
  ...
  const dataForPdfAndCsv = processedData.map((l) => [
    l.id, l.title, l.objective, l.category, l.keySkill, l.createdBy, l.createdAt, l.isActive ? 'Activa' : 'Inactiva',
  ]);
  ...
  switch (format) {
    ...
    case 'pdf':
      sendPdfTableResponse(res, buildTimestampedFilename(filenameBase, 'pdf'), {
        title: 'Lista de Lecciones', headers: headersForPdfAndCsv, rows: dataForPdfAndCsv,
      });
      break;
    default:
