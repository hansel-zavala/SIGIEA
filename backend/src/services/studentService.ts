...
export const createStudent = async (data: any) => {
  ...
  if (guardianCreds.length) {
    ...
    if (duplicates.length)
      throw new GuardianValidationError(
        `Correos duplicados: ${[...new Set(duplicates)].join(", ")}`,
      );

    const existingUsers = await studentRepository.findUsersByEmails(emails);
    if (existingUsers.length)
      throw new GuardianValidationError(
        `Correo en uso: ${existingUsers.map((u) => u.email).join(", ")}`,
      );
  }
  ...
  for (const g of guardians) {
    ...
    if (!exists) {
      const missing = ["nombres", "apellidos", "telefono", "parentesco"].filter(
        (f) => !g[f],
      );
      if (missing.length)
        throw new GuardianValidationError(
          `Faltan datos para guardián ${g.numeroIdentidad}: ${missing.join(", ")}`,
        );
    }
  }
  ...
  for (const g of guardianCreds) {
    ...
    const existing = await studentRepository.findGuardianByDni(
      g.numeroIdentidad,
    );
...
export const getAllStudents = async (query: any, user: any) => {
  ...
  if (search) {
    ...
    where.OR = [
      {
        AND: terms.map((t) => ({
          OR: [{ nombres: { contains: t } }, { apellidos: { contains: t } }],
        })),
      },
      {
        therapist: {
  ...
  const [students, total] = await studentRepository.findAndCountAll(
    where,
    skip,
    limitNum,
  );
...
export const updateStudent = async (id: number, data: any) => {
  ...
  [
    ...
    "updatedAt",
    "fullName",
  ].forEach((k) => delete studentData[k]);
  ...
  if (therapistId) {
    ...
    if (parseInt(therapistId) !== current.therapistId) {
      for (const session of current.therapySessions) {
        const conflict = await studentRepository.findConflictingSessions(
  ...
  return studentRepository.update(id, {
    ...
    therapist: therapistId
      ? { connect: { id: parseInt(therapistId) } }
      : { disconnect: true },
  });
};
...
export const addGuardianToStudent = async (studentId: number, data: any) => {
  ...
  if (
    (data.parentesco === "Padre" || data.parentesco === "Madre") &&
    student.guardians.some((g) => g.parentesco === data.parentesco)
  ) {
    throw new GuardianValidationError(
      `Ya existe un ${data.parentesco} registrado.`,
    );
  }
  ...
  if (data.email) {
    ...
    if (existingUser)
      throw new GuardianValidationError("El correo ya está en uso.");
  }

  return {
    message:
      "Funcionalidad movida al servicio (implementación detallada pendiente por brevedad)",
  };
};
...
export const exportStudents = async (query: any, user: any, res: Response) => {
  ...
  const processedData = students.map((s) => {
    ...
    return {
      id: s.id,
      fullName: `${s.nombres} ${s.apellidos}`,
      fullnametherapist: s.therapist
        ? `${s.therapist.nombres} ${s.therapist.apellidos}`
        : "N/A",
