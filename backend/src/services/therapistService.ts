...
const DEFAULT_PERMISSIONS = [
  PermissionType.VIEW_STUDENTS, PermissionType.EDIT_STUDENTS, PermissionType.MANAGE_SESSIONS,
  PermissionType.VIEW_REPORTS, PermissionType.CREATE_REPORTS, PermissionType.MANAGE_DOCUMENTS,
  PermissionType.VIEW_GUARDIANS, PermissionType.VIEW_EVENTS, PermissionType.VIEW_THERAPISTS,
  PermissionType.VIEW_MATRICULA, PermissionType.VIEW_TEMPLATES, PermissionType.VIEW_DASHBOARD,
  PermissionType.VIEW_CONTROLS, PermissionType.VIEW_DOCUMENTS, PermissionType.UPLOAD_FILES,
...
export const createTherapist = async (data: any) => {
  ...
  const existingProfile = await therapistRepository.findByIdentityNumber(identityNumber);
  ...
  const createData: Prisma.TherapistProfileCreateInput = {
    ...profileData,
    nombres, apellidos, email, identityNumber, lugarNacimiento, direccion, specialty,
    hireDate: hireDate ? new Date(hireDate) : new Date(),
    identityCardUrl, resumeUrl,
    workDays: workDays || ["Lunes", "Martes", "Miércoles", "Jueves", "Viernes"],
    workEndTime, lunchStartTime, lunchEndTime,
    user: {
      create: {
        email,
    ...
    permissions: {
      create: DEFAULT_PERMISSIONS.map(p => ({ permission: p, granted: true }))
    }
  };
...
export const getAllTherapists = async (query: any, user: any) => {
  ...
  const [therapists, total] = await therapistRepository.findAndCountAll(whereCondition, skip, limitNum);
...
export const getTherapistById = async (id: number) => {
  ...
  const assignedStudents = therapist.assignedStudents.map(student => {
    const { 
        ...
        inclusionEscolar, educacionFisica, ...rest 
    } = student as any; 
    ...
    return {
      ...
      tipoAtencion: {
        atencionGrupal, atencionIndividual, atencionPrevocacional, 
        atencionDistancia, terapiaDomicilio, atencionVocacional, 
        inclusionEscolar, educacionFisica,
      },
    };
  });
...
export const updateTherapist = async (id: number, data: any) => {
  ...
  if (profileData.hireDate) profileData.hireDate = new Date(profileData.hireDate);

  return therapistRepository.updateProfile(id, {
    email, nombres, apellidos, ...profileData
  });
};
...
export const exportTherapists = async (query: any, res: Response) => {
  ...
  const dataRows = processedData.map(t => [
    t.id, t.fullName, t.identityNumber, t.specialty, t.email, t.createAt, t.isActive
  ]);

  if (format === 'excel') {
    await sendExcelResponse(res, buildTimestampedFilename(filenameBase, 'xlsx'), headers, processedData);
  } else if (format === 'pdf') {
    sendPdfTableResponse(res, buildTimestampedFilename(filenameBase, 'pdf'), {
      title: 'Lista de Terapeutas', headers: headers.map(h => h.header), rows: dataRows
    });
  } else {
...
export const exportAssignedStudents = async (id: number, format: string, res: Response) => {
  ...
  const processedData = therapist.assignedStudents.map((s) => {
    const types = [
        s.atencionIndividual && 'Individual', s.atencionGrupal && 'Grupal',
        s.atencionPrevocacional && 'Prevocacional', s.atencionDistancia && 'A distancia',
        s.terapiaDomicilio && 'Domicilio', s.atencionVocacional && 'Vocacional',
        s.inclusionEscolar && 'Inclusión Escolar', s.educacionFisica && 'Educación Física',
  ...
  const dataRows = processedData.map(s => [
    s.id, s.fullName, s.dateOfBirth, s.jornada, s.genero, s.tiposDeAtencion
  ]);
